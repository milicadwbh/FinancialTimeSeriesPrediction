...
def predict(cal):
	...
	path = 'purchaseData' + str(minPurchases) + 'OrMore2019'
	dir = os.path.join(os.getcwd(), path)
	resultsPath = 'predictionResults'
	resultsDir = os.path.join(os.getcwd(), resultsPath)
	timestamp = time.time()

	resultsFilename = os.path.join(resultsDir, "predictionResultsForDate" + date.strftime("%Y-%m-%d") + "-" + str(timestamp) + " " + str(minPurchases) + " or more in training set" + ".txt")
	...
	for filename in glob.glob(os.path.join(dir, '*.csv')):
		if "PurchasePeriodsCustNo" in filename:
			...
			endIndex = filename.index("ItemNo")
			custNo = filename[startIndex:endIndex]

			startIndex = filename.index("ItemNo") + 6
			endIndex = filename.index(".csv")
			itemNo = filename[startIndex:endIndex]

			i = i + 1
			line = "Iteration " + str(i) + " Customer " + custNo + " and Item " + itemNo
			print(line)
			...
			allX = df['PurchasePeriod'].to_numpy()
			#print(allX)

			# transform string date to date and then compare it to entered date
			df['InvDateCurr'] = df['InvDateCurr'].map(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").date())
			#display(df[0:5])

			df = df.loc[df['InvDateCurr'] <= date]
			#display(df[:])
	...
	if (countPredictedPurchase > 0) :
		...
		line = "No predicted purchases"
	print(line)
	resultsFile.write(line + "\n")

	if (countRealizedPurchases > 0) :
